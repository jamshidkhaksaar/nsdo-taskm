...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (userRoleNameUpper === "ADMIN") {
      this.logger.log(
        `PermissionsGuard Granted (Admin): User ${user.id} (Role: ${user.role.name}) automatically granted access.`
      );
      return true;
    }

    // Ensure user role permissions are loaded for non-admins
    if (!user.role.permissions) {
        this.logger.error(
            `PermissionsGuard Error: user.role.permissions not found on request object for non-admin user ${user.id}.`,
        );
        throw new InternalServerErrorException(
