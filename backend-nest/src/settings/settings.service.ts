...
export class SettingsService {
  ...
  async initializeSettings() {
    ...
    const notificationSettingsCount =
      await this.notificationSettingsRepository.count();
  ...
  async updateApiSettings(
    id: number,
    updateApiSettingsDto: UpdateApiSettingsDto,
  ): Promise<ApiSettings> {
  ...
  async updateSecuritySettings(
    id: number,
    updateSecuritySettingsDto: UpdateSecuritySettingsDto,
  ...
  async updateSettings(
    ...
    this.logger.debug(
      `Entering updateSettings with DTO: ${JSON.stringify(updateSettingsDto)}`,
    );
    ...
    if (!updateSettingsDto || !Array.isArray(updateSettingsDto.settings)) {
      ...
      throw new Error(
        'Invalid settings data format. Expected an object with a "settings" array.',
      );
    }

    this.logger.debug(
      `Number of settings to process: ${updateSettingsDto.settings.length}`,
    );

    for (const settingDto of updateSettingsDto.settings) {
      this.logger.debug(
        `Processing setting DTO: ${JSON.stringify(settingDto)}`,
      );
      if (!settingDto || !settingDto.key) {
        this.logger.warn(
          "Skipping setting DTO due to missing key.",
          settingDto,
        );
        continue;
      }
      ...
      this.logger.debug(
        `Found existing setting for key ${settingDto.key}: ${!!setting}`,
      );

      if (setting) {
        this.logger.debug(
          `Updating setting ${settingDto.key} from ${setting.value} to ${settingDto.value}`,
        );
        ...
        this.logger.debug(
          `Creating new setting for key ${settingDto.key} with value ${settingDto.value}`,
        );
      ...
      try {
        ...
        this.logger.debug(
          `Successfully saved setting for key ${settingDto.key}`,
        );
        updatedSettings.push(savedSetting);
      } catch (dbError) {
        this.logger.error(
          `Database error saving setting for key ${settingDto.key}: ${dbError.message}`,
          dbError.stack,
        );
        throw dbError;
      }
    ...
    this.logger.debug(
      `Finished updateSettings. Returning ${updatedSettings.length} updated settings.`,
    );
  ...
  async testSendGridSettings(
    recipientEmail: string,
  ): Promise<{ success: boolean; message: string }> {
    this.logger.debug(
      `Attempting to send test email to ${recipientEmail} via @sendgrid/mail.`,
    );
    ...
    if (!apiKey) {
      ...
      return {
        success: false,
        message: "SendGrid API Key is not configured in settings.",
      };
    }
    if (!fromAddress) {
      ...
      return {
        success: false,
        message: "Default From Email Address is not configured in settings.",
      };
    }
    ...
    try {
      ...
      this.logger.log(
        `Successfully sent SendGrid test email to ${recipientEmail} via @sendgrid/mail.`,
      );
      return {
        success: true,
      ...
      this.logger.error(
        `Failed to send SendGrid test email to ${recipientEmail} via @sendgrid/mail: ${error.message}`,
        error.stack,
      );
      let errorMessage =
        "Failed to send test email. Check backend logs for details.";
      ...
      if (error.response?.body?.errors) {
        ...
        errorMessage =
          "Network error: Could not reach SendGrid. Check DNS and connectivity.";
      } else if (
        error.message.includes("Forbidden") ||
        error.statusCode === 403
      ) {
        errorMessage =
          "SendGrid Error: Authentication failed. Check your API Key permissions.";
      } else if (error.statusCode === 401) {
        errorMessage =
          "SendGrid Error: Authentication failed. Check your API Key.";
      }
