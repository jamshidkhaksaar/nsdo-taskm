...
export class ProfileService {
  ...
  async updateProfile(userId: string, profileData: any) {
    try {
      ...
      const allowedFields = ['username', 'email', 'bio', 'avatarUrl', 'skills', 'socialLinks'];
      ...
      if (profileData.first_name !== undefined || profileData.last_name !== undefined) {
        const firstName = profileData.first_name || user.username.split(' ')[0] || '';
  ...
  async verifyPassword(userId: string, password: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error verifying password for user ${userId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async updatePassword(userId: string, newPassword: string): Promise<void> {
    try {
      ...
      this.logger.error(`Error updating password for user ${userId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async updateSettings(userId: string, settingsData: any): Promise<any> {
    try {
      ...
      await this.usersService.updateUser(userId, { preferences: updatedPreferences });
      ...
      this.logger.error(`Error updating settings for user ${userId}: ${error.message}`, error.stack);
      throw error;
    }
  }
} 