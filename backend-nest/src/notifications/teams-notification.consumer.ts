...
export class TeamsNotificationConsumer implements OnModuleInit {
  ...
  private subscribeToNotifications() {
    ...
    this.redisSubscriber.on("message", (receivedChannel, message) => {
      if (receivedChannel === channel) {
        this.logger.log(
          `Received message from Redis channel ${receivedChannel} for Teams processing`,
        );
        try {
  ...
  private async handleNotification(notification: any): Promise<void> {
    ...
    if (!teamsWebhookUrl) {
      this.logger.error(
        "Teams Webhook URL (TEAMS_WEBHOOK_URL) not configured in environment variables.",
      );
      return;
    }
    ...
    try {
      ...
      const errorMessage = error.response
        ...
        : error.message;
      this.logger.error(
        `Failed to send notification to Teams: ${errorMessage}`,
        error.stack,
      );
    }
  }
