...
export class TeamsNotificationConsumer implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.redisSubscriber.on("message", (channel, message) => {
      if (channel === "notifications:new") {
        // Ensure handling the correct channel
    ...
    if (this.redisSubscriber.status === "ready") {
      this.logger.log(
        "Redis subscriber already connected. Listening for Teams notifications...",
      );
    } else {
      this.logger.warn(
        "Redis subscriber not yet connected when TeamsConsumer initialized.",
      );
  ...
  private async handleRedisMessage(message: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to process or send notification to Teams: ${errorMessage}`,
        error.stack,
      );
    }
  }
