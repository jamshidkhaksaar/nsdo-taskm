...
export class TwoFactorService {
  ...
  async getStatus(userId: string): Promise<{ enabled: boolean, method?: string }> {
    try {
      ...
      this.logger.log(`2FA status for user ${userId}: ${user.twoFactorEnabled}, method: ${user.twoFactorMethod}`);
      return { 
  ...
  async setup(userId: string, enabled: boolean, method: string = 'app'): Promise<{ qr_code?: string, message?: string }> {
    try {
      const user = await this.usersRepository.findOne({ where: { id: userId } });
      ...
      this.logger.log(`Setting up 2FA for user ${userId} (${user.username}), enabled: ${enabled}, method: ${method}`);
      ...
      // Enable 2FA
      if (enabled === true) {
        this.logger.log(`Enabling 2FA for user ${userId} with method ${method}`);
      ...
      this.logger.warn(`Unexpected case in 2FA setup for user ${userId}: enabled=${enabled}, current status=${user.twoFactorEnabled}`);
      return {};
    } catch (error) {
  ...
  async verify(userId: string, verificationCode: string, rememberBrowser: boolean = false): Promise<boolean> {
    try {
      const user = await this.usersRepository.findOne({ where: { id: userId } });
      ...
      this.logger.log(`Verifying 2FA code for user: ${user.username}, method: ${user.twoFactorMethod}`);
      ...
      if (verified) {
        ...
        if (rememberBrowser) {
          this.logger.log(`Remember browser requested for user ${user.username}`);
          ...
          this.logger.log(`Added remembered browser for user ${user.username}, expires: ${expiresAt}`);
        }
  ...
  async sendEmailCode(userId: string, email: string): Promise<void> {
    try {
      ...
      this.logger.log(`Sending verification code ${verificationCode} to ${email} for user ${user.username}`);
      ...
      if (this.mailService) {
        try {
          ...
          this.logger.log(`Verification code email sent to ${email} for user ${user.username}`);
        } catch (mailError) {
          this.logger.error(`Failed to send email: ${mailError.message}`, mailError.stack);
          // Continue even if mail sending fails - we'll just log the code for testing
          this.logger.warn(`TEST ONLY - verification code is: ${verificationCode}`);
        }
      } else {
        // No mail service available, just log the code
        this.logger.warn(`Mail service not available - TEST ONLY - verification code is: ${verificationCode}`);
      }
    } catch (error) {
      this.logger.error(`Failed to send verification code: ${error.message}`, error.stack);
      throw error;
    }
  }
  
  async checkRememberedBrowser(userId: string, fingerprint: string): Promise<boolean> {
    try {
      this.logger.log(`Checking remembered browser for user ${userId}, fingerprint: ${fingerprint?.substring(0, 15)}...`);
      ...
      if (remembered) {
        this.logger.log(`Found remembered browser for user ${user.username}, fingerprint: ${fingerprint?.substring(0, 15)}...`);
        return true;
      }
      
      this.logger.log(`No matching remembered browser found for user ${user.username}`);
      return false;
    } catch (error) {
      this.logger.error(`Failed to check remembered browser: ${error.message}`, error.stack);
      return false;
    }
  }
} 