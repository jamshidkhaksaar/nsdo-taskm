import { Injectable, UnauthorizedException, InternalServerErrorException, Logger, Inject, forwardRef, BadRequestException } from '@nestjs/common';
...
export class AuthService {
  ...
  async signIn(loginCredentialsDto: LoginCredentialsDto): Promise<LoginSuccessResponse | TwoFactorRequiredResponse> {
    ...
    try {
      ...
      if (user.twoFactorEnabled) {
        this.logger.log(`2FA is enabled for user: ${username}. Returning 2FA required response.`);
      ...
      this.logger.log(`2FA is not enabled for user: ${username}. Generating tokens...`);
  ...
  async login2FA(userId: string, verificationCode: string): Promise<LoginSuccessResponse> {
    ...
    if (!user.twoFactorEnabled || !user.twoFactorSecret) {
      this.logger.warn(`login2FA: 2FA is not enabled or secret is missing for user: ${user.username}`);
    ...
    */
    this.logger.log(`TOTP check for user ${user.username}: ${isCodeValid ? 'Valid' : 'Invalid'}`);

    // TODO: Step 2: If TOTP failed, check if it's a valid recovery code
    if (!isCodeValid) {
      this.logger.log(`TOTP code invalid, checking recovery codes for user: ${user.username}`);
    ...
    if (!isCodeValid) {
      this.logger.warn(`Invalid 2FA code (TOTP or Recovery) for user: ${user.username}`);
      throw new UnauthorizedException('Invalid Two-Factor Authentication code.');
    }

    // If code is valid (either TOTP or Recovery), generate tokens
    this.logger.log(`2FA verification successful for user: ${user.username}. Generating tokens...`);
    const payload: JwtPayload = { username: user.username, sub: user.id };
    const accessToken = this.jwtService.sign(payload);
    const refreshToken = this.jwtService.sign(payload, { expiresIn: '7d' });
