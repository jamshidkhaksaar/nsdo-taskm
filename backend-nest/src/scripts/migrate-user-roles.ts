...
async function migrateUserRoles() {
  ...
  console.log(
    `Connecting to database: ${dbHost}:${dbPort}/${dbDatabase} as ${dbUsername}`,
  );
  ...
  try {
    ...
    // Get count of users
    const [totalUsersRows] = await connection.query(
    ...
    usersToUpdate.forEach((user) => {
      ...
      console.log(
        `Will update user ${user.username} (${user.id}) from '${user.role}' to '${newRole}'`,
      );
    });
    ...
    if (error.code === "ER_ACCESS_DENIED_ERROR") {
      console.error(
        "Database authentication failed. Check your DB_USERNAME and DB_PASSWORD in .env file.",
      );
    }
  } finally {
    if (connection) {
