...
export class AnalyticsService {
  ...
  async getProductivityMetrics(userId: number) {
    ...
    const onTimePercentage =
      totalCompleted > 0 ? (completedOnTime / totalCompleted) * 100 : 0;
  ...
  async getCompletionRate(userId: number, period = "weekly") {
    ...
    const completedCountQuery = this.taskRepository
      ...
      .where("user.id = :userId", { userId })
      .andWhere("task.status = :status", { status: TaskStatus.COMPLETED })
      .andWhere("task.updatedAt BETWEEN :startDate AND :now", {
        startDate,
        now,
      });
    ...
    const totalCountQuery = this.taskRepository
      ...
      .leftJoin("task.assignedToUsers", "user")
      .where("user.id = :userId", { userId })
      .andWhere("task.createdAt BETWEEN :startDate AND :now", {
        startDate,
        now,
      });
    ...
    if (completedTasks.length > 0) {
      ...
      avgCompletionTime =
        totalTimeMs / completedTasks.length / (1000 * 60 * 60 * 24); // in days
    }
  ...
  async getTaskRecommendations(userId: number) {
    ...
    const averageCompletionDays =
      await this.calculateAverageCompletionDays(userId);
    ...
    const tasksToStart = await this.taskRepository
      ...
      .where("user.id = :userId", { userId })
      .andWhere("task.status = :status", { status: TaskStatus.PENDING })
      .andWhere("task.dueDate BETWEEN :now AND :dueDateThreshold", {
        now,
        dueDateThreshold,
      })
      .orderBy("task.dueDate", "ASC")
